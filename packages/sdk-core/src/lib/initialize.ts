...
export const initialize = async (
  ...
  try {
    // ??? comment next line to enable multiple responses
  ...
  if (c.collectConsoleErrors) {
    logger.debug("Starting collecting errors");
    const errorLogger = ErrorsLogger.getInstance();
    errorLogger.startCollectingErrors();
  }
